<?php
	...
	// $con = mysqli_connect("localhost","root","mysql","app_desktop");
	$con2 = mysqli_connect("localhost","root","mysql","app_client");
	...
	// lbrcetak = 21.8 & tgcetak = 30.5 & jmlcetak = 500 & bahan = 1 & jmlwarna = 4 & jmlwarna2 = 0 & lam = 0 & finishing1 = 0 & finishing2 = 0 & finishing3 = 0 & finishing4 = 0 & finishing5 = 0 & finishing6 = 0 & finishing7 = 0 & finishing8 = 0 & finishing9 = 66 & lbrf1 = 1 & tgf1 = 1 & lbrf2 = 1 & tgf2 = 1 & lbrf3 = 1 & tgf3 = 1 & lbrf4 = 1 & tgf4 = 1 & lbrf5 = 1 & tgf5 = 1 & lbrf6 = 1 & tgf6 = 1 & lbrf7 = 1 & tgf7 = 1 & lbrf8 = 1 & tgf8 = 1 & lbrf9 = NaN & tgf9 = 1 & jmlset = 1 & modul = brosur & insheet = 1 & jml_satuan = 1 & cetak_bagi = Y & ket_satuan = lbr & ongpot = Y & j_mesin =  & kethitung =
	...
	function hitungcetak($lbrcetak,$tgcetak,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$finishing2, $finishing1, $finishing3,$finishing4,$finishing5,$finishing6,$finishing7,$finishing8,$finishing9,$finishing10,$submit,$lbrf1,$tgf1,$lbrf2,$tgf2,$lbrf3,$tgf3,$lbrf4,$tgf4,$lbrf5,$tgf5,$lbrf6,$tgf6,$lbrf7,$tgf7,$lbrf8,$tgf8,$lbrf9,$tgf9,$lbrf10,$tgf10,$modul,$jml_satuan,$cetak_bagi,$ket_satuan,$ongpot,$j_mesin,$kethitung,$jilid,$ongkos_lipat,$pakeplat,$persen){
		...
		if (empty($finishing1) or $finishing1==0) {
			...
			$sqlfinishing1 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing1'");
			$row5=mysqli_fetch_array($sqlfinishing1);    
		...
		if (empty($finishing2) or $finishing2==0) {
			...
			$sqlfinishing2 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing2'");
			$row5=mysqli_fetch_array($sqlfinishing2);    
		...
		if (empty($finishing3) or $finishing3==0) {
			...
			$sqlfinishing3 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing3'");
			$row5=mysqli_fetch_array($sqlfinishing3);    
		...
		if (empty($finishing4) or $finishing4==0) {
			...
			$sqlfinishing4 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing4'");
			$row5=mysqli_fetch_array($sqlfinishing4);    
		...
		if (empty($finishing5) or $finishing5==0) {
			...
			$sqlfinishing5 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing5'");
			$row5=mysqli_fetch_array($sqlfinishing5);    
		...
		if (empty($finishing6) or $finishing10==6) {
			...
			$sqlfinishing6 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing6'");
			$row6=mysqli_fetch_array($sqlfinishing6);    
		...
		if (empty($finishing7) or $finishing7==0) {
			...
			$sqlfinishing7 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing7'");
			$row7=mysqli_fetch_array($sqlfinishing7);    
		...
		if (empty($finishing8) or $finishing8==0) {
			...
			$sqlfinishing8 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing8'");
			$row8=mysqli_fetch_array($sqlfinishing8);    
		...
		if (empty($finishing9) or $finishing9==0) {
			...
			$sqlfinishing9 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing9'");
			$row9=mysqli_fetch_array($sqlfinishing9);    
		...
		if (empty($finishing10) or $finishing10==0) {
			...
			$sqlfinishing10 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$finishing10'");
			$row10=mysqli_fetch_array($sqlfinishing10);    
	...
	global $con2;
		...
		// Cari Biaya Potong
		$sql3 = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='29'");
		$row3 = mysqli_fetch_array($sql3); // pilih biaya potong
		$hargapot  =$row3['HargaLebih'];	
		$hargapotmin  =$row3['HargaMin'];
		
		// Cari Jenis Bahan
		$sqljenis = mysqli_query($con2,"select * from kategori_bahan where id_kategori='$bahan'");
		$rowjenis = mysqli_fetch_array($sqljenis); // 
		...
		$sqlam = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='$laminat'");
		$row7 = mysqli_fetch_array($sqlam);
		...
		$sql = mysqli_query($con2,"select * from mesin where kdmesin='$mesin' AND aktif='Y'");
		while($row=mysqli_fetch_array($sql)){ //mesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesin 
			...
			while ($ulangbb <= 3) {  //selama cetak bolak balik  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
				...
				while ($z / $lebarpot >= 1){ // Selama Tinggi Mesin masih muat u/ Lebar Cetakan	// x aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa	
					...
					while ($z / $tinggipot >= 1) { //  Selama Tinggi Mesin masih muat u/ Tinggi Cetakan  // y
						...
						$sql2 = mysqli_query($con2,"select * from tbl_bahan where id_kategori='$bahan' and publish='Y' and Harga_Bahan > '0'");			
						while($row2=mysqli_fetch_array($sql2)){ //vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
							...
							$sqlinsheet = mysqli_query($con2,"select * from tbl_insheet where '$jmlbhn' between dari and sampai");			
							$rowinsheet=mysqli_fetch_array($sqlinsheet);
							...
							// }
							if ($jmlrealcetak > $replatsetiap && $replatsetiap > 1) 
							{
								$replat = ceil($jmlrealcetak / $replatsetiap);
							} 
							else {
							...
							if($jenis_mesin == 'PrintDigital' ){
								...
								$sql = mysqli_query($con2,"select * from harga_print where kdmesin='$mesin' AND '$jmlprint' between jml_min and jml_max");
								$row=mysqli_fetch_array($sql);  // mesin yg di ceklis
							...
							if(!empty($ongkos_lipat)){
								$sqllipat = mysqli_query($con2,"select * from tbl_biaya where KdBiaya='52'");
								$row10=mysqli_fetch_array($sqllipat);    
								$hrgfinishing10min  	=$row10['HargaMin'];
								$hrgfinishing10lebih  	=$row10['HargaLebih'];
								$jmlfinishing10min  	=$row10['JumlahMin'];	
								$nmfinishing10			=$row10['Nama_Biaya'];
	...
	// // Cari Jenis Bahan
	// $sqljenis = mysqli_query("select * from kategori_bahan where id_kategori='$bahan'");
	// $rowjenis = mysqli_fetch_array($sqljenis); // 
	// $jenis_bahan  =$rowjenis['nama_kategori'];	
	...
	// $sql2 = mysqli_query("select * from tbl_bahan where id_kategori='$bahan' and publish='Y' and Harga_Bahan> '0'");			
	// while($row2=mysqli_fetch_array($sql2)){ //vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
